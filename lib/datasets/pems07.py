...
class Pems07(PandasDataset):
    def __init__(self):
        df, dist, mask = self.load()
        self.dist = dist
        super().__init__(dataframe=df, u=None, mask=mask, name="pems07", freq='5T', aggr='nearest')

    def load(self, impute_zeros=True):
        path = os.path.join(datasets_path["pems07"], 'PEMS07_wrapped.csv')
    ...
    def load_distance_matrix(self, ids):
        path = os.path.join(datasets_path["pems07"], 'pems07_dist.npy')
        try:
            dist = np.load(path)
        except:
            distances = pd.read_csv(os.path.join(datasets_path["pems07"], 'PEMS07.csv'))
            num_sensors = len(ids)
...
class MissingValuesPems07(Pems07):
    def __init__(self, p_fault=0.0015, p_noise=0.05, mode="random"):
        super(MissingValuesPems07, self).__init__()
